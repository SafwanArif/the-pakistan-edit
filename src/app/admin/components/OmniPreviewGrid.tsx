...
const PreviewItem = React.memo<{ p: any, displayDraft: Draft, currentStep: number, totalSlides: number }>(({ p, displayDraft, currentStep, totalSlides }) => {
...
interface OmniPreviewGridProps {
    draft: Draft;
    currentStep: number;
}

const FALLBACK_IMAGE = "https://images.unsplash.com/photo-1590409279774-7221bfad3dd9?q=80&w=1200&auto=format&fit=crop";

/**
 * 2027 Institutional Standard: OmniPreviewGrid
 * Optimized for high-speed narrative ingestion with sub-pixel precision scaling.
 */
export const OmniPreviewGrid: React.FC<OmniPreviewGridProps> = ({ draft, currentStep }) => {
    ...
    const platformConfigs = useMemo(() => {
        ...
        return platforms.map(platform => {
            const config = OMNI_CONFIG[platform];
            return {
                id: platform,
